...
settings_table = Table(prefixed("settings"), metadata, Column("id", Integer, primary_key=True), Column("data", Text, nullable=False), Column("updated_at", DateTime(timezone=True), nullable=False))
logs_table = Table(prefixed("logs"), metadata, Column("id", Integer, primary_key=True, autoincrement=True), Column("level", String(20), nullable=False), Column("message", Text, nullable=False), Column("created_at", DateTime(timezone=True), nullable=False))
...
DEFAULT_SETTINGS: Dict[str, Any] = {
    "enabled": False, "times": ["08:40"], "band_key": "", "band_name": "", "dcu_count": 1, "modem_counts": [1, 1, 1, 1],
    "date_format": "%Y년 %m월 %d일", "modem_template": "{date} {zone}권역 모뎀{team}조", "dcu_template": "{date} DCU {team}조", "tail_templates": ["{date} TBM"],
    ...
    "skip_saturday": False, "skip_korean_holidays": False, "skip_extra_dates": False, "extra_skip_dates": [],
}
...
class BandOpenApi:
    ...
    def _api_call(self, api_path: str, params: Optional[dict] = None, method: str = "get") -> dict:
        ...
        params = dict(params or {}); params["access_token"] = self.access_token
...
def render_line(template: str, context: Dict[str, Any]) -> str:
    ...
    except KeyError as exc: raise ValueError(f"템플릿 변수 오류: {exc}. 사용 가능 변수는 {{date}}, {{zone}}, {{team}}, {{type}} 입니다.") from exc
...
def build_lines(settings: Dict[str, Any], base_dt: Optional[datetime] = None) -> List[str]:
    ...
    for zone in build_sequence(4, settings["modem_zone_order"]):
        for team in build_sequence(settings["modem_counts"][zone - 1], settings["modem_team_order"]): lines.append(render_line(settings["modem_template"], {"date": date_text, "zone": zone, "team": team, "type": "모뎀"}))
    for team in build_sequence(settings["dcu_count"], settings["dcu_team_order"]): lines.append(render_line(settings["dcu_template"], {"date": date_text, "zone": "", "team": team, "type": "DCU"}))
    for template in settings["tail_templates"]: lines.append(render_line(template, {"date": date_text, "zone": "", "team": "", "type": "TBM"}))
...
def acquire_run(run_key: str) -> bool:
    try:
        with engine.begin() as conn: conn.execute(insert(run_history_table).values(run_key=run_key, status="STARTED", detail="", created_at=now_kst()))
        return True
...
def get_last_run() -> Optional[dict]:
    with engine.begin() as conn: row = conn.execute(select(run_history_table).order_by(run_history_table.c.id.desc()).limit(1)).mappings().first()
...
def post_lines(settings: Dict[str, Any], lines: List[str]) -> None:
    ...
    for index, line in enumerate(lines, start=1):
        ...
        while True:
            try:
                api.create_post(settings["band_key"], line, do_push=settings["do_push"]); add_log("INFO", f"게시 성공 ({index}/{len(lines)}): {line}")
                if index < len(lines) and settings["post_interval_seconds"] > 0: time.sleep(settings["post_interval_seconds"])
                break
            except Exception as exc:
                attempts += 1; add_log("ERROR", f"게시 실패 ({index}/{len(lines)}, {attempts}회): {line} / {exc}")
...
def execute_schedule(scheduled_hhmm: str, manual: bool = False) -> None:
    ...
    base_dt = now_kst(); run_key = f"{'manual' if manual else 'schedule'}:{base_dt.strftime('%Y%m%d')}:{scheduled_hhmm}:{settings.get('band_key', '')}"
    if not acquire_run(run_key): add_log("WARNING", f"중복 실행 방지로 건너뜀: {run_key}"); return
    try:
        skip_reason = get_skip_reason(settings, base_dt)
        if skip_reason and not manual:
            update_run(run_key, "SKIPPED", skip_reason); add_log("INFO", f"예약 실행 건너뜀: {scheduled_hhmm} / {skip_reason}"); return
        lines = build_lines(settings, base_dt)
        if not lines: raise RuntimeError("생성된 게시글이 없습니다. 조 수 또는 템플릿을 확인하세요.")
        add_log("INFO", f"실행 시작: {scheduled_hhmm}, 총 {len(lines)}건"); post_lines(settings, lines); update_run(run_key, "SUCCESS", f"{len(lines)}건 게시 완료"); add_log("INFO", f"실행 완료: {scheduled_hhmm}, 총 {len(lines)}건")
    except Exception as exc:
        update_run(run_key, "FAILED", str(exc)); add_log("ERROR", f"실행 실패: {scheduled_hhmm} / {exc}")
...
def dashboard_status() -> Dict[str, Any]:
    settings = get_settings(); runtime = get_runtime_values(); heartbeat_row = runtime.get("worker_heartbeat"); heartbeat_value = heartbeat_row["value"] if heartbeat_row else ""; heartbeat_age: Optional[int] = None; worker_alive = False
    if heartbeat_value:
        try:
    ...
    return {"app": APP_NAME, "now": now_kst().strftime("%Y-%m-%d %H:%M:%S"), "token_ready": api.is_ready, "db": "PostgreSQL" if DATABASE_URL.startswith("postgresql") else "SQLite/local", "enabled": settings["enabled"], "jobs": worker_jobs if worker_jobs else next_run_estimates(settings), "last_run": get_last_run(), "today_skip_reason": get_skip_reason(settings), "worker_heartbeat": heartbeat_value, "worker_heartbeat_age": heartbeat_age, "worker_alive": worker_alive, "worker_state": runtime.get("worker_state", {}).get("value", "대기"), "worker_last_reload": runtime.get("worker_last_reload", {}).get("value", "")}
